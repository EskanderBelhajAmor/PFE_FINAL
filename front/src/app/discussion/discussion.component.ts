...
export class DiscussionComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.candService.getCandidaturesByEmployeurId(this.id)
      .subscribe(
        (data) => {
          this.cands = data;
        },
  ...
  selectOffer(employeurid: string, candid: string, offerId: string, prenomrecep: string): void {
    ...
    if (this.selectedemployeurid && this.selectedcandid) {
      ...
      this.discussionForm.patchValue({
        ...
        emetteurId: this.selectedemployeurid,
        recepteurId: this.selectedcandid,
        ...
        .subscribe(
          (messages) => {
            ...
            this.messages = messages.map((message: any) => ({
              ...message,
              isSent: message.emetteurId === this.selectedemployeurid
            }));
          },
          (error) => {
